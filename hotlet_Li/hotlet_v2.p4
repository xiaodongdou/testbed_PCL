    ...
    inout ingress_intrinsic_metadata_for_tm_t        ig_tm_md)
{
    ...
    bit<1> recir_flag = 0;
    bit<8> clear_count = 0;
    //debug --- store the index in the reg for get freq and id in bfshell
    Register<bit<16>, bit<16>>(0x1) index_reg;
    RegisterAction<bit<16>, bit<16>, bit<16>>(index_reg) index_store = 
    {
        void apply(inout bit<16> register_data, out bit<16> result){
            register_data = meta.index;
        }
    };

    action index_store_a(){
        index_store.execute(0);
    }
    ...
    table index_store_t{
        ...
        default_action = index_store_a;
    }

    Register<bit<8>,bit<16>>(0x1) flag_reg;
    RegisterAction<bit<8>,bit<16>,bit<8>>(flag_reg) flag_store = 
    {
    ...
    table flag_store_t{
        ...
        default_action = flag_reg_a;
    }

    Register<bit<8>,bit<16>>(0x1) ecmp_reg;
    RegisterAction<bit<8>,bit<16>,bit<8>>(ecmp_reg) ecmp_store = {
        void apply(inout bit<8> register_data, out bit<8> result){
            register_data = register_data + (bit<8>)recir_flag;
        }
    };
    
    action ecmp_check_a(){
        ecmp_store.execute(0);
    }
    table ecmp_check_t{
        actions = {
            ecmp_check_a;
        }
        default_action = ecmp_check_a;
    }

    Register<bit<32>,bit<16>>(0x1) recir_reg;
    RegisterAction<bit<32>,bit<16>,bit<32>>(recir_reg) recir_count = {
        void apply(inout bit<32> register_data, out bit<32> result){
            register_data = register_data + (bit<32>)clear_count;
        }
    };

    action recirulate_count(){
        recir_count.execute(0);
    }
    table recirulate_count_t{
        actions = {
            recirulate_count;
        }
        default_action = recirulate_count;
    }

    Register<bit<32>,bit<16>>(0x1) over_reg;
    RegisterAction<bit<32>,bit<16>,bit<32>>(over_reg) over_count = {
        void apply(inout bit<32> register_data, out bit<32> result){
            register_data = register_data + (bit<32>)meta.over_flag;
        }
    };

    action over_count_a(){
        over_count.execute(0);
    }
    table over_count_t{
        actions = {
            over_count_a;
        }
        default_action = over_count_a;
    }
    ...
    RegisterAction<bit<16>, bit<16>, bit<16>>(port_reg) output_port = 
    {
        void apply(inout bit<16> register_data, out bit<16> result){
            ...
            if(meta.flag >= 7) register_data = meta.port;//new id or (id matched and ts freq both >H )
    ...
    {
        void apply(inout bit<32> register_data, out bit<1> result){
            register_data = meta.over_ts_int;
   ...
   table sketch_id_t{
    ...
    action get_dif_over(){
        meta.over_ts_int = meta.timestamp_now - meta.timestamp_last;
    }

    table get_dif_over_t{
        actions = {
            get_dif_over;
        }
        default_action = get_dif_over;
    }
    ...
    apply{
        ...
        if(hdr.arp.isValid()){
            ...
            //insert id and freq
            sketch_id_t.apply();
            id_match_t.apply(); //identy id matched set flag[2:2]
            id_identy_t.apply();//identy new id 
            ...
            if(hdr.ipv4.res == 0 && meta.flag < 8){//new id 不用检查是否超时
                //overtime check
                get_dif_over_t.apply();//change timestamp dif int
                overtime_check_t.apply();//register set meta.over_flag 1
            }

            // outport process --- change outport in register or set outport to send
            output_port_t.apply();
            flag_store_t.apply();//debug
            recirulate_count_t.apply();//debug
            ecmp_check_t.apply();//debug
            over_count_t.apply();
    ...
    inout egress_intrinsic_metadata_for_output_port_t  eg_oport_md)
{
    ...
    apply {
        if(hdr.ipv4.ecn == 1 || hdr.ipv4.ecn ==2){
            if(eg_intr_md.enq_qdepth - ECN_MARK_THRESHOLD >= 0){
                mark_ecn_t.apply();
            }
        }
    }
